...
//사용 안함, redis cluster 접속시 이걸 사용하면 됨
public class RedisClusterPanel extends JPanel implements RedisPanelInterface {
	 ...
	 * Create the panel.
	 */
	public RedisClusterPanel(MainFrame frame) {
		this.frame = frame;
		...
		dbComboBox.setEnabled(false); //redis cluster 일때는 DB 사용 중지 
		...
		searchResultRemoveButton.addActionListener(new ActionListener() {
		    public void actionPerformed(ActionEvent e) {
		        RedisClusterPanel.this.searchResultRemove();
		    }
		});
		...
		allRemoveButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				RedisClusterPanel.this.removeAllKeyInCache();
			}
		});
		...
		btnNewButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				new Thread(() -> {
					...
					RedisClusterPanel.this.getKeyList(true, "");
	...
	//public void connectRedis(String hosts, String password, boolean applyBase64Encoding) {	    
	public void connectRedis(String hosts, String port, String password, String database, String numOfDatabase, boolean applyBase64Encoding) {
		this.hosts = hosts;
		this.password = password;
		this.applyBase64Encoding = applyBase64Encoding;

		redisServerNameLabel.setText("> [" + hosts + "] ");
	...
	private void connectRedisServer() {
		if(!redisConnectFlag) {
			...
			//hosts = "123.0.0.1:8888,124.0.0.1:9999";
			...
			for(String host: hostArray) {
			    ...
			    redisURI.setHost(hostPort[0]);
			    redisURI.setPort(Integer.parseInt(hostPort[1]));
...
//			}
	...
	private void removeKeyInCache(int selectedRow) {
		...
		if(result == 0) {
			try {
				...
				if(deletedCount > 0) {
					System.out.println("\nRedis Server: ["+this.hosts+"] {Key: "+key+"} removed!!");
				}
	...
	private void removeAllKeyInCache() {
		...
		int result = JOptionPane.showConfirmDialog(RedisClusterPanel.this,
		...
		if(result == 0) {
			try {
				...
				if(!StringUtils.isEmpty(flushResult)) {
					System.out.println("\nRedis Server: ["+this.hosts+"] All Keys removed!!");
				}
	...
	private void printCacheValue(String key, String value) {
		...
		System.out.println("\nRedis Server: ["+this.hosts+"] {Key: "+key+"}");
		System.out.println("-------------------------------------");
    ...
    public List<String> getKeyList(boolean isRefreshButtonClicked, String searchKey) {
		...
		try {
			...
			System.out.println("\nRedis Server: ["+this.hosts+"] Search Result(Key) Count -> " + size);
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	private void redisDisconnectedProcessing() {
		System.out.println("\nRedis Server: ["+this.hosts+"] Disconnected!!");
	...
	class ButtonEditor extends DefaultCellEditor {
		...
		public Object getCellEditorValue() {
			if (isPushed) {
				...
				case 1:
					new Thread(() -> RedisClusterPanel.this.printCacheValue(selectedRow)).start();
					break;				
				case 2:
					RedisClusterPanel.this.removeKeyInCache(selectedRow);
					break;
				default:
					break;
				}
	...
	private void searchResultRemove() {
        ...
        if(result == 0) {
            ...
            try {
                ...
                if(deletedCount > 0) {
                    System.out.println("\nRedis Server: ["+this.hosts+"] {Total: "+deletedCount+" Keys} removed!!");
                }
            } catch (Exception e) {
                e.printStackTrace();
                frame.closeProgressBar();
